...
clean_df_matched <- data.frame("Submitted_Name" = grep( 'cf.', clean_df$Submitted_Name, value = T, invert = T )) 
...
# "Ficus variegata Blume" and "Diospyros pilosanthera Blanco" are counted twice and must be removed, label issue as "double count"
double_counts   <- c( "Ficus variegata Blume",
...
# "Xanthophyllum flavescens Roxb." is duplicated as a synonym and must be removed, label issue as "synonym"
synonyms        <- c("Xanthophyllum flavescens Roxb.")
...
taxa_out <- read.csv( 'results/palanan_taxa.csv' , header = TRUE)
taxa_unresvd <- read.csv( 'results/palanan_taxa_unresvd.csv' , header = TRUE)

# Select variables for demographic means tables - one for clean taxa and another for unresolved taxa
# Get demographic variables from palanan_means
demog_means_df         <- select( palanan_means, -c( genus, family, IDlevel ) ) %>%
...
write.csv( demog_means_df_clean, 'results/palanan_demog_means.csv',
           row.names = F )
write.csv( demog_means_df_unresvd, 'results/palanan_demog_means_unresvd.csv',
           row.names = F )

# Select variables for median tables - one for clean taxa and another for unresolved taxa
# Get demographic variables from palanan_medians
demog_medians_df         <- select( palanan_medians, -c( genus, family, IDlevel ) ) %>%
  rename( Submitted_Name = latin, Sp_Code = sp )

#Capitalise taxa in demog_medians_df
demog_medians_df <- upper_case_genus( demog_medians_df )

# Join clean taxa to rest of schema via accepted names and remove four duplicate taxa
...
write.csv( demog_medians_df_clean, 'results/palanan_demog_medians.csv',
           row.names = F )
write.csv( demog_medians_df_unresvd, 'results/palanan_demog_medians_unresvd.csv',
           row.names = F )


# Analyse relationship between sample size and CI width
# Create variable for CI width
palanan_means <- palanan_means %>% mutate( "growth_layer_1_CI90_width"   = growth_layer1_CI.95 - growth_layer1_CI.05,
...
growth_layer_1_graph <- ggplot( data = subset( palanan_means, growth_layer_1_imputed != TRUE), aes( x = growth_layer_1_obs, y = growth_layer_1_CI90_width ) ) + geom_point() + labs( x = "sample size", y = "90% CI width", title = "Growth Layer 1")
growth_layer_2_graph <- ggplot( data = subset( palanan_means, growth_layer_2_imputed != TRUE), aes( x = growth_layer_2_obs, y = growth_layer_2_CI90_width ) ) + geom_point() + labs( x = "sample size", y = "90% CI width", title = "Growth Layer 2")
growth_layer_3_graph <- ggplot( data = subset( palanan_means, growth_layer_3_imputed != TRUE), aes( x = growth_layer_3_obs, y = growth_layer_3_CI90_width ) ) + geom_point() + labs( x = "sample size", y = "90% CI width", title = "Growth Layer 3")
growth_layer_4_graph <- ggplot( data = subset( palanan_means, growth_layer_4_imputed != TRUE), aes( x = growth_layer_4_obs, y = growth_layer_4_CI90_width ) ) + geom_point() + labs( x = "sample size", y = "90% CI width", title = "Growth Layer 4")
grid.arrange( growth_layer_1_graph,
              growth_layer_2_graph,
              growth_layer_3_graph,
              growth_layer_4_graph )

survival_layer_1_graph <- ggplot( data = subset( palanan_means, survival_layer_1_imputed != TRUE), aes( x = survival_layer_1_obs, y = survival_layer_1_CI90_width ) ) + geom_point() + labs( x = "sample size", y = "90% CI width", title = "Survival Layer 1")
survival_layer_2_graph <- ggplot( data = subset( palanan_means, survival_layer_2_imputed != TRUE), aes( x = survival_layer_2_obs, y = survival_layer_2_CI90_width ) ) + geom_point() + labs( x = "sample size", y = "90% CI width", title = "Survival Layer 2")
survival_layer_3_graph <- ggplot( data = subset( palanan_means, survival_layer_3_imputed != TRUE), aes( x = survival_layer_3_obs, y = survival_layer_3_CI90_width ) ) + geom_point() + labs( x = "sample size", y = "90% CI width", title = "Survival Layer 3")
survival_layer_4_graph <- ggplot( data = subset( palanan_means, survival_layer_4_imputed != TRUE), aes( x = survival_layer_4_obs, y = survival_layer_4_CI90_width ) ) + geom_point() + labs( x = "sample size", y = "90% CI width", title = "Survival Layer 4")
